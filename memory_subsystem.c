...
void memory_handle_l1_miss(uint32_t address)  
{
  ...
  //CODE HERE
    uint32_t e_address;
    uint32_t e_data[WORDS_PER_CACHE_LINE];
    l1_insert_line(address, read_data, &e_address, e_data, &status);
  ...
  //CODE HERE
    if (status & (0x1)) {
      l2_cache_access(e_address, e_data, WRITE_ENABLE_MASK, NULL, &status);
      if (!(status & (0x1))) {
        memory_handle_l2_miss(e_address, WRITE_ENABLE_MASK);
        l2_cache_access(e_address, e_data,WRITE_ENABLE_MASK, NULL, &status);
      }
    }
}
...
void memory_handle_l2_miss(uint32_t address, uint8_t control)
{
  ...
  //CODE HERE
    ...
    uint32_t e_data[WORDS_PER_CACHE_LINE];
    uint8_t status = 0;
    l2_insert_line(address, cache_line, &e_address, e_data, &status);
  ...
  //CODE HERE
    if (status & (0x1)) {
      main_memory_access(e_address, e_data, WRITE_ENABLE_MASK, NULL);
    }
}

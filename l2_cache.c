...
void l2_initialize()
{
  ...
  // CODE HERE
    for (uint32_t i = 0; i < L2_NUM_CACHE_SETS; i++) {
      for (uint32_t j = 0; j < L2_LINES_PER_SET; j++) {
		     ...
		     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  //CODE HERE
    uint32_t index_bit=(address& L2_ADDRESS_INDEX_MASK) >> L2_ADDRESS_INDEX_SHIFT;
  ...
  //CODE HERE
    uint32_t tag_bit = (address & L2_ADDRESS_TAG_MASK) >> L2_ADDRESS_TAG_SHIFT;
  ...
  //CODE HERE
    L2_CACHE_SET *s = &l2_cache[index_bit];
    L2_CACHE_ENTRY *e = NULL;
  
    for (uint32_t i = 0; i < L2_LINES_PER_SET; i++) {
      uint32_t v_r_d_tag = s -> lines[i].v_r_d_tag;
        
      if (!(v_r_d_tag & L2_VBIT_MASK) &&(v_r_d_tag & L2_ENTRY_TAG_MASK) != tag_bit) {
    ...
    e ->v_r_d_tag= ( e ->v_r_d_tag)|(L2_RBIT_MASK);
    
    if (control & READ_ENABLE_MASK) {
      for (uint32_t j = 0; j < WORDS_PER_CACHE_LINE; j++) {
        read_data[j] = e -> cache_line[j];
      }
    }

    if (control & WRITE_ENABLE_MASK) {
      for (uint32_t j = 0; j < WORDS_PER_CACHE_LINE; j++) {
        e ->cache_line[j] = write_data[j];
      }
      e ->v_r_d_tag=(e ->v_r_d_tag)| L2_DIRTYBIT_MASK;
    }
		    ...
		    uint8_t *status)
{
  ...
  //LOOP STARTS HERE
    for (uint32_t i = 0; i < L2_LINES_PER_SET; i++) {
       ...
       // CODE HERE
        ...
        if ((v_r_d_tag & L2_VBIT_MASK) == 0) {
          for (uint32_t k = 0; k < WORDS_PER_CACHE_LINE; k++) {
            s->lines[i].cache_line[k] = write_data[k];
          }
          s->lines[i].v_r_d_tag = 0;
          s->lines[i].v_r_d_tag |= tag_bit;
          s->lines[i].v_r_d_tag |= L2_VBIT_MASK;
          *status &= ~0x1;
            return;}
       ...
       // CODE HERE
            ...
            if ((v_r_d_tag & L2_RBIT_MASK) == 0 && (v_r_d_tag & L2_DIRTYBIT_MASK) == 0 && r0_d0_index == NOT_FOUND) {
              r0_d0_index = i;}
            if ((v_r_d_tag & L2_RBIT_MASK) == 0 && (v_r_d_tag & L2_DIRTYBIT_MASK) > 0 && r0_d1_index == NOT_FOUND) {
              r0_d1_index = i;}
            if ((v_r_d_tag & L2_RBIT_MASK) > 0 && (v_r_d_tag & L2_DIRTYBIT_MASK)  == 0 && r1_d0_index == NOT_FOUND) {
              r1_d0_index = i;
            }
          }
        }
  ...
  // CODE HERE
    L2_CACHE_SET *s = &l2_cache[index_bit];
    L2_CACHE_ENTRY *e2 = &s->lines[evict];
   
    if (e2->v_r_d_tag & L2_DIRTYBIT_MASK) {
      *evicted_writeback_address =
          ((e2->v_r_d_tag & L2_ENTRY_TAG_MASK) << L2_ADDRESS_TAG_SHIFT) |
          (index_bit << L2_ADDRESS_INDEX_SHIFT);
  ...
  // CODE HERE
    for (uint32_t i = 0; i < WORDS_PER_CACHE_LINE; i++) {
          evicted_writeback_data[i] = e2->cache_line[i];}
    *status |= 0x1;
      } else {
        *status &= ~0x1;}
  ...
  // CODE HERE
    for (uint32_t i = 0; i < WORDS_PER_CACHE_LINE; i++) {
      e2->cache_line[i] = write_data[i];
    }
    e2->v_r_d_tag = 0;
    e2->v_r_d_tag |= tag_bit;
    e2->v_r_d_tag |= 1 << 31;
  }
...
void l2_clear_r_bits()
{
  ...
  //CODE HERE
    for (uint32_t i = 0; i < L2_NUM_CACHE_SETS; i++) {
      for (uint32_t j = 0; j < L2_LINES_PER_SET; j++) {
        l2_cache[i].lines[j].v_r_d_tag=(l2_cache[i].lines[j].v_r_d_tag)&  (~(L2_RBIT_MASK));
      }
    }
}

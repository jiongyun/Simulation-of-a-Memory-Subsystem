		     ...
		     uint8_t control, uint32_t *read_data, uint8_t *status)
{
  ...
  //CODE HERE
    unsigned int index = (address & L1_ADDRESS_INDEX_MASK) >> L1_ADDRESS_INDEX_SHIFT;
  ...
  //CODE HERE
    unsigned int tag = (address & L1_ADDRESS_TAG_MASK) >> L1_ADDRESS_TAG_SHIFT;
  ...
  //CODE HERE
    unsigned int offset = (address & L1_ADDRESS_WORD_OFFSET_MASK) >> L1_ADDRESS_WORD_OFFSET_SHIFT;
  ...
  //CODE HERE
    L1_CACHE_ENTRY *e = &l1_cache[index];
    if ((e->v_d_tag & L1_VBIT_MASK) == 0 ||
        (e->v_d_tag & L1_ENTRY_TAG_MASK) != tag) {
  ...
  //CODE HERE
    if (control & READ_ENABLE_MASK) {
        *read_data = e->cache_line[offset];
    }
    if (control & WRITE_ENABLE_MASK) {
        e->cache_line[offset] = write_data;
        e->v_d_tag |= L1_DIRTYBIT_MASK;
    }
		    ...
		    uint8_t *status)
{
  ...
  //CODE HERE
    L1_CACHE_ENTRY *e = &l1_cache[index];
    if ((e->v_d_tag & L1_VBIT_MASK) && (e->v_d_tag & L1_DIRTYBIT_MASK)) {
        *evicted_writeback_address = ((e->v_d_tag & L1_ENTRY_TAG_MASK) << L1_ADDRESS_TAG_SHIFT) | (index << L1_ADDRESS_INDEX_SHIFT);
        for (unsigned int i = 0; i < WORDS_PER_CACHE_LINE; i++) {
          evicted_writeback_data[i] = e->cache_line[i];
        }
      *status |= 0x1;
    }
